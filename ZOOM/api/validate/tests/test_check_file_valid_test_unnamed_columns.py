...
class FileValidTestCase(TestCase):
    ...
    def test_check_file_valid_test_unnamed_columns(self):

        with open('samples/check_file_valid_test_unnamed_columns.csv') as fp:
                res_file_upload = self.c.post(
                        ...
                        'file_name': 'check_file_valid_test_unnamed_columns.csv',
                        })
        ...
        self.assertEquals(res_check_file_valid.json()['error'], "Cannot validate, unnamed columns in data set or unessecary text at start of file.")
