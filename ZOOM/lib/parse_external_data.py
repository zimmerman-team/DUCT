...
file_dict = {
    WB: {
        ...
        WB,
        "description":
        "Indicator data from WB (upload done through automation).",
        "file_tags": ["WB"],
        "mapping": {
        ...
        "input_path":
        "../scripts/formatters/" + WB + "/input/",
        "output_path":
        "../scripts/formatters/" + WB + "/output/",
        "conversion":
        "../scripts/formatters/" + WB + "/",
        "columns": [
            DEFAULT_INDICATOR_COLUMN, "Indicator Name", "Country Code", "1960",
    ...
    CRS: {
        ...
        CRS,
        "description":
        "Aggregated CRS from OECD (upload done through automation).",
        ...
        "mapping": {
            ...
            'country': ['Recipient'],
            'measure_value': [
                'Currency', 'Commitments', 'Disbursements', 'Commitments Defl',
                'Disbursements Defl'
            ],
            'date_value': ['Year'],
            'source': [],
            'other': [],
            'indicator_filter': [
                'Sector', 'Aid Type', 'Income Group', 'Purpose', 'Donor',
                'Finance Type', 'Commitments', 'Disbursements',
                'Commitments Defl', 'Disbursements Defl'
            ],
            'empty_unit_of_measure': {
                'Currency': 'Number',
                'Disbursements': 'Number',
                'Disbursements Defl': 'Number',
                'Commitments': 'Number',
                'Commitments Defl': 'Number'
            }
        },
        "input_path":
        "../scripts/formatters/" + CRS + "/input/",
        "output_path":
        "../scripts/formatters/" + CRS + "/output/",
        "conversion":
        "../scripts/formatters/" + CRS + "/",
        "columns": [
            "Year", "DonorName", "RecipientName", "IncomegroupName",
            "FlowName", "Finance_t", "Aid_t", "usd_commitment",
            "usd_disbursement", "usd_commitment_defl", "usd_disbursement_defl",
            "CurrencyCode", "PurposeName", "SectorName"
        ],
        "nice_name": {
            "Year": "Year",
            "DonorName": "Donor",
        ...
        "measure_value": [
            "Commitments", "Disbursements", "Commitments Defl",
            "Disbursements Defl"
        ]
    }
}
...
def start_mapping(file_choice):
    ...
    for file_name in file_list:
        ...
        with open(path + file_name, 'rb') as fp:
            res_file_upload = c.post(
                URL + 'file/?format=json', {
                    ...
                    file_name,
                    'description':
                    (file_name + " " + file_dict[file_choice]["description"]),
                    'file_name':
                    file_name
                })
        ...
        print('id ', id)
        res = requests.patch(URL + 'file/{}/?format=json'.format(id),
                             headers=headers,
                             data=(json.dumps(patch_data)))  # """"""
        print("Update file: ", res)

        res = c.post(URL + 'validate/?format=json', {"id": id}, format='json')
        print("Validation: ", res)

        res = c.post(URL + 'manual-mapper/?format=json', {
            "id": id,
        ...
        res = c.post(URL + 'file/update_status/?format=json', {
            "id": id,
            "status": 5
        })
    ...
    for file_name in file_list:
        ...
        if (size > SPLIT_SIZE):  # over 5.5mb split
            ...
            for i in range(split_range):
                data_split[i].to_csv(file_dict[file_choice]["output_path"] +
                                     file_name[:-4] + "(" + str(i) + ")" +
                                     ".csv",
                                     sep=',',
                                     index=False)
...
def convert_data(file_choice):
    ...
    for file_name in file_list:
        print('File being converted: ',
              file_dict[file_choice]["input_path"] + file_name)
        data = pd.read_csv(file_dict[file_choice]["input_path"] + file_name,
                           sep=character_sep[file_choice])
        ...
        if "nice_name" in file_dict[file_choice]:
            data.rename(columns=file_dict[file_choice]["nice_name"],
                        inplace=True)
        ##check column width and size an split accoringly
        data.to_csv(file_dict[file_choice]["output_path"] + file_name[:-4] +
                    ".csv",
                    sep=',',
                    index=False)
...
def flatten_data(file_choice):
    ...
    aid_t_conv = pd.read_csv(file_dict[file_choice]["conversion"] +
                             "aid_types.csv")
    aid_t_conv = pd.Series(aid_t_conv.Name.values,
                           index=aid_t_conv.Code).to_dict()
    currency_t_conv = pd.read_csv(file_dict[file_choice]["conversion"] +
                                  "currency_types.csv")
    currency_t_conv = pd.Series(currency_t_conv.Name.values,
                                index=currency_t_conv.Code).to_dict()
    finance_t_conv = pd.read_csv(file_dict[file_choice]["conversion"] +
                                 "finance_types.csv")
    finance_t_conv = pd.Series(finance_t_conv.Name.values,
                               index=finance_t_conv.Code).to_dict()
