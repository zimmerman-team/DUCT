...
def get_geolocation_dictionary():
    dict = {}
    dict['iso2'] = list(Geolocation.objects.values_list('iso2', flat=True).distinct())
    dict['iso2'] = [x for x in dict['iso2'] if x is not None]
    value_dict = {k.lower(): 'iso2' for k in dict['iso2']}
    dict['iso3'] = list(Geolocation.objects.values_list('iso3', flat=True).distinct())
    ...
    for entry in list(Geolocation.objects.distinct('type')):
        dict[entry.type] = list(Geolocation.objects.filter(type=entry.type).values_list('tag', flat=True).distinct())
        dict[entry.type] = [x for x in dict[entry.type] if x is not None]
        value_dict = {**value_dict, **{k.lower(): entry.type for k in dict[entry.type]}}
    return value_dict
